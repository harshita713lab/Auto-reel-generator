...
export const useAnimation = (config) => {
    ...
    const { durationInFrames, delay = 0, easing = Easing.inOut(Easing.ease), from = { opacity: 0, scale: 0.8 }, to = { opacity: 1, scale: 1 }, } = config;
    ...
    for (const key in from) {
        if (key in to) {
            result[key] = interpolate(easedProgress, [0, 1], [from[key], to[key]]);
        }
    }
    return {
        progress,
...
export const useSpringAnimation = (config) => {
    const frame = useCurrentFrame();
    const { durationInFrames, delay = 0, from = { opacity: 0, scale: 0.8 }, to = { opacity: 1, scale: 1 }, } = config;
    const progress = Math.max(0, Math.min((frame - delay) / durationInFrames, 1));
    // Spring-like easing
...
export const useSequenceAnimation = (configs) => {
    const frame = useCurrentFrame();
    const results = [];
    let totalDuration = 0;
