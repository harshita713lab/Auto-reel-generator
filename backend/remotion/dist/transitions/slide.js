...
export const SlideTransition = ({ children, durationInFrames = 30, direction = 'left', distance = 100, delay = 0, }) => {
...
export const SlideTransitionInOut = ({ children, durationInFrames = 30, direction = 'left', outDirection, distance = 100, delay = 0, }) => {
    const frame = useCurrentFrame();
    const progress = Math.max(0, Math.min((frame - delay) / durationInFrames, 1));
    const eased = Easing.inOut(Easing.ease)(progress);
    ...
    if (eased <= 0.5) {
        ...
        switch (outDir) {
            case 'left':
                x = interpolate(p, [0, 1], [0, -distance]);
                break;
            case 'right':
                x = interpolate(p, [0, 1], [0, distance]);
                break;
            case 'up':
                y = interpolate(p, [0, 1], [0, -distance]);
                break;
            case 'down':
                y = interpolate(p, [0, 1], [0, distance]);
                break;
