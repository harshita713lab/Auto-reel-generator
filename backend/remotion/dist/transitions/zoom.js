...
export const ZoomTransition = ({ children, durationInFrames = 30, direction = 'both', scale = 1.3, delay = 0, }) => {
...
export const ZoomRotateTransition = ({ children, durationInFrames = 30, direction = 'both', scale = 1.3, rotation = 15, delay = 0, }) => {
    const frame = useCurrentFrame();
    const progress = Math.max(0, Math.min((frame - delay) / durationInFrames, 1));
    const eased = Easing.inOut(Easing.ease)(progress);
    let currentScale = 1;
