...
function nextAnimation(index) {
    return animations[index % animations.length];
}
function nextTransition(index) {
    return transitions[index % transitions.length];
}
export function generateScenes(images, beatTimestamps) {
    const scenes = [];
    const fps = 30;
    // Convert beat timestamps (in seconds) to frame numbers
    const beatFrames = (beatTimestamps && Array.isArray(beatTimestamps) && beatTimestamps.length > 0)
