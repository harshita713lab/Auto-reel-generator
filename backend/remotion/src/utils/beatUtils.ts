 ...
 */
export function getBeatScale(frame: number, fps: number = 30, beatTimestamps?: number[]): number {
  const beats: number[] = (beatTimestamps && Array.isArray(beatTimestamps) && beatTimestamps.length > 0)
    ? beatTimestamps
