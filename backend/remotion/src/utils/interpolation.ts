 ...
 */
export const interpolateArray = (values: number[], t: number): number => {
...
export const interpolatePoints = (points: Point[], t: number): Point => {
 ...
 */
export const interpolateOvershoot = (
  t: number,
  from: number,
  to: number,
  overshoot: number = 0.2
): number => {
...
export const interpolateTypes = {
  ...
  easeIn: (a: number, b: number, t: number) => {
    const x = clamp(t, 0, 1);
    return lerp(a, b, x * x);
  },
  easeOut: (a: number, b: number, t: number) => {
    const x = clamp(t, 0, 1);
    return lerp(a, b, 1 - (1 - x) * (1 - x));
  },
  easeInOut: (a: number, b: number, t: number) => {
    const x = clamp(t, 0, 1);
